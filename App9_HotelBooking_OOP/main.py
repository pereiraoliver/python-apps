...
df = pandas.read_csv(BASE_DIR / "hotels.csv", dtype={"id": str})
df_cards = pandas.read_csv(BASE_DIR / "cards.csv", dtype=str).to_dict(orient="records")
df_cards_security = pandas.read_csv(BASE_DIR / "card_security.csv", dtype=str)
...
class Hotel:
    ...
    def book(self):
        ...
        df.to_csv(BASE_DIR / "hotels.csv", index=False)
...
if hotel.available():
    ...
    if credit_card.validate(expiration="12/26", holder="JOHN SMITH", cvc="123"):
        if credit_card.authenticate(given_password="mypass"):
            ...
            spa_booking = input("Do you want to book a spa package? (yes/no) ")
            if spa_booking == "yes":
